...
def dashboardPage() {
	...
	dynamicPage(name: "dashboardPage", title: "Dashboard", uninstall: true, nextPage: generalSetupPage,submitOnChange: true) {
		...
		section("ABOUT") {
			paragraph "${get_APP_NAME()}, the smartapp that enables Heating/Cooling Zoned Solutions based on your ecobee schedule(s)- coupled with smart vents (optional) for better temp settings control throughout your home"
			paragraph "Version 7.1.7\n" + 
	...
	dynamicPage(name: "configDisplayPage", title: "Running Schedule(s) Config", nextPage: generalSetupPage,submitOnChange: true) {
		...
		section("General") {
			...
			paragraph " >SetVentSettings: ${setVentSettingsString}" +
					...
					"\n >AwayOrPresentNow: ${state?.programHoldSet}" + 
...
private boolean is_alternative_cooling_efficient(outdoorTemp, outdoorHum) {
	...
	traceEvent(settings.logFilter,"is_alternative_cooling_efficient>outdoorTemp In Farenheits=$outdoorTempInF",settings.detailedNotif)
		...
		if ((currentMode in ['cool','off', 'auto']) && ((outdoorTemp) &&
			...
			if ((!useAlternativeWhenCooling) && (currentMode != 'off')) {                
				traceEvent(settings.logFilter,"check_use_alternative_cooling>about to turn off the thermostat $thermostat, saving the current thermostat's mode=$currentMode",
                	settings.detailedNotif,get_LOG_WARN(),true)            
...
private void check_if_hold_justified() {
	...
	if ((currentSetClimate?.toUpperCase()=='AWAY')  && (residentPresent)) {
		if ((state?.programHoldSet == 'Away') && (!currentProgName.toUpperCase().contains('AWAY'))) {       
			thermostat.resumeThisTstat()
			reset_state_program_values()
			traceEvent(settings.logFilter,"check_if_hold_justified>it's not been quiet since ${state.programSetTimestamp},resume ecobee program...", settings.detailedNotif,
				get_LOG_INFO(),true)
		...
		if ((state?.programHoldSet == 'Away') && ((currentProgName.toUpperCase().contains('AWAY')) ||
			...
			thermostat.resumeThisTstat()
			reset_state_program_values()
			traceEvent(settings.logFilter,"'Away' hold no longer needed, resumed program to ecobee ${currentProgName} schedule",settings.detailedNotif,get_LOG_INFO(),true)
                
		} else if (state?.programHoldSet != 'Away')  {
			traceEvent(settings.logFilter,"quiet since ${state.programSetTimestamp}, current ecobee schedule= ${currentProgName}, 'Away' hold justified",settings.detailedNotif,get_LOG_INFO())
			set_main_tstat_to_AwayOrPresent('away')
		}    
	...
	if (ecobeeMode == 'cool') {
		...
		if (((less_cool_threshold) && (outdoorTemp > less_cool_threshold.toFloat())) && 
			...
			traceEvent(settings.logFilter,"Reverting lessCool hold, ${less_cool_threshold} < outdoorTemp <${more_cool_threshold}",settings.detailedNotif,get_LOG_INFO(),true)
			...
			if ((actual_temp_diff > max_temp_diff) && (!state?.programHoldSet)) {
				traceEvent(settings.logFilter,"Hold differential too big (${actual_temp_diff}), needs adjustment back to baseline cool setpoint",settings.detailedNotif,get_LOG_INFO(),true)
				thermostat.setCoolingSetpoint( state?.scheduleCoolSetpoint)
			}
		}
	} else if (ecobeeMode.contains('heat')) {
		traceEvent(settings.logFilter,"check_if_hold_justified>evaluate: programHeatTemp= ${programHeatTemp} vs.avgIndoorTemp= ${avg_indoor_temp}",settings.detailedNotif, get_LOG_INFO())
		if (((more_heat_threshold) && (outdoorTemp > more_heat_threshold.toFloat())) && 
			...
			traceEvent(settings.logFilter,"Reverting lessHeat Hold, ${less_heat_threshold} < outdoorTemp > ${more_heat_threshold}",detailedNotif,get_LOG_INFO(),true)
			...
			traceEvent(settings.logFilter,"eval: actual_temp_diff ${actual_temp_diff} between program heating setpoint & hold setpoint vs. max temp diff ${max_temp_diff}",
				settings.detailedNotif,get_LOG_INFO())            
			if ((actual_temp_diff > max_temp_diff) && (!state?.programHoldSet)) {
				traceEvent(settings.logFilter,"Hold differential too big ${actual_temp_diff}, needs adjustment back to baseline heat setpoint",settings.detailedNotif,get_LOG_INFO(),true)
				thermostat.setHeatingSetpoint( state?.scheduleHeatSetpoint)
			}
		}
	}
...
private def adjust_tstat_for_more_less_heat_cool(indiceSchedule) {
	...
	if (currentMode.contains('heat')) {
		if ((moreHeatThreshold != null) & (outdoorTemp <= moreHeatThreshold?.toFloat()))  {
			...
			thermostat.setHeatingSetpoint(targetTstatTemp)
			traceEvent(settings.logFilter,"heating setPoint now= ${targetTstatTemp}, outdoorTemp > ${lessHeatThreshold}",settings.detailedNotif, get_LOG_INFO(),true)
		}            
	}
	if (currentMode== 'cool') {
		...
		if ((moreCoolThreshold!= null) && (outdoorTemp >= moreCoolThreshold?.toFloat())) {
			...
			traceEvent(settings.logFilter,"cooling setPoint now= ${targetTstatTemp}, outdoorTemp <${lessCoolThreshold}",settings.detailedNotif, get_LOG_INFO(),true)
			thermostat.setCoolingSetpoint(targetTstatTemp)
		}            
	} 
}
...
private def adjust_thermostat_setpoint_in_zone(indiceSchedule) {
	...
	traceEvent(settings.logFilter,"adjust_thermostat_setpoint_in_zone>schedule ${scheduleName},all temps collected from sensors=${indoor_all_zones_temps}",settings.detailedNotif)
	...
	if (adjustmentFanFlag) {
		...
		if ((max_temp_diff >0) && (temp_diff.abs() > max_temp_diff)) {
			traceEvent(settings.logFilter,"schedule ${scheduleName},avg_temp_diff=${temp_diff.abs()} > ${max_temp_diff} :adjusting fan mode as temp differential in zone is too big",
				settings.detailedNotif,get_LOG_INFO(),true)
			// set fan mode with overrideThreshold=true
			set_fan_mode(indiceSchedule, true)          
                
		} else {
			traceEvent(settings.logFilter,"schedule ${scheduleName},avg_temp_diff=${temp_diff.abs()} < ${max_temp_diff} :adjusting fan mode to auto",
				settings.detailedNotif,get_LOG_INFO(),true)
			thermostat.fanAuto()        
		}        
	}
